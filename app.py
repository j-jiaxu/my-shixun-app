...
# 词云
def plot_ciyun_chart(word_count):
    plt.title('词云图')
    # 将字典转为列表的形式
    ...
    wordcloud = (  
        WordCloud()  
    .add("", ls, word_size_range=[30, 30+len(word_count)],shape='circle')  
...
def pa_sidebar(selected_option,word_count):
    ...
    elif selected_option == "词云图":
        plot_ciyun_chart(word_count)
...
def main():
    ...
    st.sidebar.title("实训")
    #侧边栏选项
    list_baidu_project= ['折线图', '饼图', '柱形图','词云图','散点图','雷达图','漏斗图']
    ...
    if url:
        ...
        # 可视化
        pa_sidebar(selected_option,word_count)
